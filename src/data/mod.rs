...
impl PassiveTree {
    pub fn compute_positions_and_stats(&mut self) {
        for (_, node) in self.nodes.iter_mut() {
            ...
            // fill name, is_notable, stats from skill table
...
mod tests {
    ...
    #[test]
    fn test_collect_stats_from_real_tree() {
        let tree = PassiveTree::load_tree("data/POE2_TREE.json");

        let mut stats_summary = vec![
            ("maximum_life", "Maximum Life", 0, 0.0),
            ("evasion", "Evasion", 0, 0.0),
            ("maximum_mana", "Mana", 0, 0.0),
            ("strength", "Strength", 0, 0.0),
            ("dexterity", "Dexterity", 0, 0.0),
        ];

        for node in tree.nodes.values() {
            if let Some(skill_id) = &node.skill_id {
                if let Some(skill) = tree.passive_skills.get(skill_id) {
                    for stat_name in skill.stats.keys() {
                        for stat in stats_summary.iter_mut() {
                            if stat_name.contains(stat.0) {
                                stat.2 += 1; // Increment count
                                stat.3 += skill.stats[stat_name]; // Add value
                            }
                        }
                    }
                }
        ...
        for (key, label, count, total) in &stats_summary {
            println!(
                "{} Nodes Count: {}, Total {}: {}",
        ...
        assert!(stats_summary[0].2 > 0, "Expected at least one life node");
        assert!(stats_summary[1].2 > 0, "Expected at least one evasion node");
        assert!(stats_summary[2].2 > 0, "Expected at least one mana node");
        assert!(
            stats_summary[3].2 > 0,
            "Expected at least one strength node"
        );
        assert!(
            stats_summary[4].2 > 0,
            "Expected at least one dexterity node"
        );
    }
}
