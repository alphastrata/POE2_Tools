...
fn rx_rpx(
    ...
    mut activate_node_with_colour: EventWriter<ManualNodeHighlightWithColourReq>,
    mut activation: EventWriter<NodeActivationReq>,
    mut cam: EventWriter<MoveCameraReq>,
    mut clear: EventWriter<ClearAllReqReq>,
    ...
    while let Ok(cmd) = server.rx.try_recv() {
        match cmd {
            ...
            Command::ClearAll => {
                clear.send(ClearAllReqReq);
            }
            Command::ActivateNodeWithColour(id, col) => {
                activate_node_with_colour.send(ManualNodeHighlightWithColourReq(id, col));
            }
                ...
                node_positions.par_iter().for_each(|(tf, nid)| {
                    if **nid == target {
                        if let Err(e) = oneshot.send(*tf) {
                            log::error!(
                                "Unable to send the transform for the node {target}'s position.\nError msg: {e}"
                            );
                        }
                    }
                });
