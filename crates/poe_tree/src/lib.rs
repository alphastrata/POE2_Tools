...
impl PassiveTree {
    ...
    pub fn remove_hidden(&mut self) {
        ...
        log::debug!(
            "Pruned tree in {duration:?}. Removed {removed_edge_count} edges and {removed_node_count} nodes."
        );
        log::debug!(
                                    ...
                                    Some(cons) => match cons.as_array() {
                                        Some(array) => array.iter().filter_map(move |connection| {
                                            match connection.get("id").and_then(|id| id.as_u64()) {
                                                ...
                                                None => {
                                                    eprintln!(
                                                        "Invalid connection in node `{from_id}`: {connection:?}"
                                                    );
                                        ...
                                        None => {
                                            eprintln!(
                "Connections field is not an array in node `{from_id}`: {cons:?}"
            );
                                            panic!()
                                        }
                                    },
                                    None => {
                                        eprintln!(
                                            "Missing connections field in node `{from_id}`: {node:?}"
                                        );
                                        panic!()
                                    }
                                }
...
pub fn get_circle_radius(radius: u8, position: NodeId, group: &GroupId) -> f32 {
    *ORBIT_RADII.get(radius as usize).unwrap_or_else(|| {
        panic!(
            "Failed to retrieve radius for r={radius} with position={position} and group coordinates=({group:#?})"
        )
    })
}
