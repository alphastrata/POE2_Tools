...
const NOTABLE_COLOR: Color = Color {
    r: 1.00,
    g: 0.86,
    b: 0.42,
    a: 1.0,
...
impl TreeVisualization {
    ...
    fn clamp_camera(&mut self, sw: f32, sh: f32) {
        ...
        let minx = self.min_x as f32;
        let maxx = self.max_x as f32;
        let miny = self.min_y as f32;
        let maxy = self.max_y as f32;
        if self.camera.pos.x < minx + halfw {
            self.camera.pos.x = minx + halfw;
        }
        if self.camera.pos.x > maxx - halfw {
            self.camera.pos.x = maxx - halfw;
        }
        if self.camera.pos.y < miny + halfh {
            self.camera.pos.y = miny + halfh;
        }
        if self.camera.pos.y > maxy - halfh {
            self.camera.pos.y = maxy - halfh;
        }
    }
    ...
    fn draw(&mut self, ctx: &mut Context) -> GameResult {
        ...
        if let Some(id) = self.hovered_node {
            if let Some(node) = self.data.passive_tree.nodes.get(&id) {
                ...
                let bg =
                    Mesh::new_rectangle(ctx, DrawMode::fill(), r, Color::from_rgba(0, 0, 0, 180))?;
